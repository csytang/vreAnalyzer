...
public class ObjDefUseModelManager {
	...
	public static abstract class AbstractDefUseModel {
		...
		/** Helper for subclasses: identifies those arguments that represent objects that can be used inside a call and adds them to given list. */
		protected void getObjArgs(InvokeExpr invExp, List<Value> objVals) {
	...
	/** Intended primarily for <init>(...) methods, which do NOT use 'this'. */
...
//			uses.add(invExp);
	...
	public static List<Value> getInternalObjDefs(InvokeExpr invExp) {
		...
		if (duModel == null) {
			// special case: <init>(...) not already in models list
			if (tgtMtd.contains(": void <init>(")) {
			...
			switch (Options.duModelBehavior()) {
				...
				case NONE: return EmptyModel.inst.getDefs(invExp);
			}
		}
	...
	public static List<Value> getInternalObjUses(InvokeExpr invExp) {
		...
		if (duModel == null) {
			// special case: <init>(...) not already in models list
			if (tgtMtd.contains(": void <init>(")) {
				System.out.println("Warning: using default use model for initializer " + tgtMtd);
				return DefThisUseAllArgsModel.inst.getUses(invExp);
			}
			
			System.out.println("Warning: use model not found for " + tgtMtd + "; using " + Options.duModelBehavior() + " behavior");
			switch (Options.duModelBehavior()) {
				case DEF_USES_ALL: return DefThisUseThisAllArgsModel.inst.getUses(invExp);
				case USES_ALL: return UseAllArgsModel.inst.getUses(invExp);
				case NONE: return EmptyModel.inst.getUses(invExp);
			}
		}
