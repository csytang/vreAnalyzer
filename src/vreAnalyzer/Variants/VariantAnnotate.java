...
public class VariantAnnotate {
    ...
    int[][]positions;
    int[]lines;
    public VariantAnnotate(List<Stmt> stmts,File htmlFile){
        ...
        for(int i = 0;i < stmts.size();i++) {
            ...
            if(startFromSource){
                HTMLAnnotation.annotatemultipleLineHTML(htmlFile, positions, Color.RED, MainFrame.inst().getHTMLToJava());
            }else{
                HTMLAnnotation.annotatemultipleLineHTML(htmlFile, lines, Color.RED, MainFrame.inst().getHTMLToJava());
            }
